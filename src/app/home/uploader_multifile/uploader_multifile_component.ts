...
// @Component({
//   templateUrl: 'uploader_multifile.html',
//   styleUrls: ['uploader_multifile.css','normalizer.css'],
// })
...
//         return fetch(`${this.ENDPOINTS.UPLOAD_STATUS}?fileName=${file.name}&fileId=${fileReq.options.fileId}`)
//           .then(res => res.json())
...
//         //  const [uploadedPerc, successCount, failedCount, pausedCount] = document.getElementById('file-progress-tracker').appendChild(uploadProgress).children;
...
//         fetch(`${this.ENDPOINTS.UPLOAD_READ_FILE}?file=${callBtn.id}`)
//         .then(async (response) => {
...
//       uploader = this.uploadFiles(uploadedFiles, {
//         onProgress,
//         onError,
//         onAbort,
//         onComplete
...
//       this.uploadAndTrackFiles(e.currentTarget.files)
